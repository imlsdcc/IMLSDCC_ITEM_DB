...
<xsl:stylesheet version="1.0"
        ...
        <xsl:element name="record">
            <xsl:element name="metadata">
              <xsl:element name="dc">
                  ...
                  <xsl:apply-templates select="cld:isLocatedAt"/>                  
    ...
    <xsl:template match="dcterms:isPartOf">
        <xsl:if test="starts-with(text(), 'http://imlsdcc.grainger.uiuc.edu/Registry/Collection/?')">
            ...
            <xsl:value-of select="document($isPartOfLink)/imlsdccProf:collectionDescription/dc:title"/>
            <xsl:text> </xsl:text>
                <xsl:value-of select="document($isPartOfLink)/imlsdccProf:collectionDescription/gen:isLocatedAt"/>
            </xsl:element>
        </xsl:if>
